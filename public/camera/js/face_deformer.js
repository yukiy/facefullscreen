...
var faceDeformer = function() {
	...
	this.load = function(element, points, pModel, vertices) {
		...
		for(var i=0; i<textureVertices.length; i+=2){
			ctx.beginPath();
			var rectx = textureVertices[i]*width;
			var recty = textureVertices[i+1]*height;
			if( rectx >= width)  rectx -= 2;
			if( recty >= height) recty -= 2;
			ctx.fillRect(rectx, recty, 2, 2);

			if(i < textureVertices.length-4){
				ctx.beginPath();
				ctx.moveTo(textureVertices[i]*width, textureVertices[i+1]*height);
				ctx.lineTo(textureVertices[i+2]*width, textureVertices[i+3]*height);
				ctx.lineTo(textureVertices[i+2]*width, textureVertices[i+3]*height);
				ctx.lineTo(textureVertices[i]*width, textureVertices[i+1]*height);
				ctx.stroke();				
			}
		}
	...
	this.drawSquareFace = function(points) {
		...
		var vertices = [];
		for (var i = 0;i < verticeMap.length;i++) {
//			console.log("num: " + verticeMap[i][0]);

			vertices.push(points[verticeMap[i][0]][0]); //verticeMap[i][0]のx
			vertices.push(points[verticeMap[i][0]][1]); //verticeMap[i][0]のy
